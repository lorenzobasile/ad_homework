...
//naive matrix multiplication for rectangular matrices
...
//Strassen's algorithm for rectangular matrices
void strassen_aux(float **C, float const *const *const A,
                  ...
                  const size_t nr_A, const size_t nc_A, const size_t nc_B)
{
    //if the matrix is very small, call naive algorithm
    ...
    //ceilings of sizes/2
    size_t nra2r = nr_A/2+nr_A%2;
    size_t nca2r = nc_A/2+nc_A%2;
    size_t ncb2r = nc_B/2+nc_B%2;
    //floors of sizes/2
    size_t nra2 = nr_A/2;
    size_t nca2 = nc_A/2;
    size_t ncb2 = nc_B/2;
    ...
    //S1 = B12 - B22
    //two separate operations are needed to obtain S1 because the matrices to be summed have different sizes
    ...
    sum_matrix_blocks(SA[0], (const float* const *const) SA[0], A,
                      ...
                      nra2r,nca2);
    //B22+padding
    //temp is zero-padded version of B22
    ...
    sub_matrix_blocks(SB[1], (const float* const *const) SB[1], B,
                      ...
                      nca2r, ncb2r);
    //A22+padding
    //once again, a temporary matrix is needed to host padded A22
...
//improved version of Strassen's algorithm with less allocations
                  ...
                  const size_t nr_A, const size_t nc_A, const size_t nc_B)
{
    ...
    //when starting, C has to be filled with zeros
    reset(C,nr_A,nc_B);
    //ceilings of sizes/2
    size_t nra2r = nr_A/2+nr_A%2;
    size_t nca2r = nc_A/2+nc_A%2;
    ...
    //now only two S matrices are needed
    ...
    //C22=P1
    //P1 is immediately copied, so that P can be reused for the following computations
    ...
    //C12=P1(C12)+P2
    //once again, P is immediately copied
    sum_matrix_blocks(C, (const float* const *const) C, (const float* const *const) P,
                      C_f_row, C_f_col+ncb2r,
                      C_f_row, C_f_col+ncb2r,
                      0, 0,
